...
module.exports = {
	bundle: function(){
    	...
    	var tasks = files.map(function(entry) {
        	return browserify({ debug: true, entries: join(config.scripts.src, entry) })
				...
				.pipe(source(join(config.scripts.dist, 'law-chart.min.js')))
	...
	lint: function() {
		gulp.src([join(config.scripts.src, '*.js'), join(config.scripts.src, '**/*.js'), '!' + join(config.scripts.src, 'utils/helpers.js')])
			.pipe(eslint())
